...
class CollectCompletedTasks:
    ...
    def add_all_completed_task_items_to_task_history(self, task_list):
        ...
        completed_task_blocks = [Block.text_block(f"{i} {j}") for i, j, k in task_list if k]
        ...
        new_entry = dict(
            ...
            properties=dict(
                Name=dict(
                    title=[
                        dict(text=dict(content=f"{datetime.now().date()}"))
                    ]
                ),
                Completion=dict(
                    rich_text= [
                        dict(
                            text=dict(
                                content=task_completion_emoji_map[len(completed_task_blocks)]
                            )
                        )
                    ]
                )
            ...
            children=[
                Block.heading_1("Completed Tasks"),
